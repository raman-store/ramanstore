...
export default function AdminShell({ children, onLogout }: { children: React.ReactNode; onLogout: () => void }) {
  ...
  const nav = [{ to: "/", label: "Overview", icon: "◫" }, { to: "/products", label: "Products", icon: "◇" }, { to: "/products/new", label: "Add product", icon: "＋" }, { to: "/orders", label: "Orders", icon: "▤" }, { to: "/sliders", label: "Homepage slider", icon: "▣" }];
  const page = location.pathname === "/" ? "Store overview" : location.pathname === "/orders" ? "Customer orders" : location.pathname === "/sliders" ? "Homepage slider" : location.pathname.includes("new") ? "Add new product" : location.pathname.includes("edit") ? "Edit product" : "Product catalogue";
